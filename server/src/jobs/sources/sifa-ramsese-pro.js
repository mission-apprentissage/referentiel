...
const ETATS = ["Ouvert", "À ouvrir"];

module.exports = (custom = {}) => {
  let name = "sifa-ramsese-pro";

  return {
    name,
    async raw() {
      let input = custom.input || (await getFromStorage("ACCE_UAI.csv"));
      return compose(
        input,
        ...
        filterData((data) => {
          return ETATS.includes(data.etat_etablissement_libe) && NATURES.includes(data.nature_uai_libe);
        })
      );
    },
    async referentiel() {
      let input = await this.raw();
